    ...
    serverRouter.connection.query(`insert into faculty_list values(${fno}, '${name}', '${gender}', '${email}', '${phone}')` , function (err, result) {
        ...
        if (err) {
            ...
            let mailOptions = {
                ...
                text: 'You have been successfully added to the faculty database. The username given for your login is '+ name +' . Click the following link to reset your password so that you can login. '+ ' http://34.121.14.80:3000/reset_pass.html'
            };
            ...
            res.redirect('http://34.121.14.80:3000/faculty_success.html');
        }
    });
});
...
router.post('/removefaculty', function (req, res) {
    ...
    serverRouter.connection.query(`delete from faculty_list where fno = ${fno}` , function(err, result){
        ...
        if (err) {
            ...
            res.redirect('http://34.121.14.80:3000/Rem_fac_success.html');
        }
    });
});
